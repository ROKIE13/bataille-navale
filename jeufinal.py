...
def attaque_ordinateur():
    global joueur_actif, joueur_cible

    # Choix case aléatoire jamais jouée
    while True:
        x = random.randint(0, NB_CASES - 1)
        y = random.randint(0, NB_CASES - 1)
    ...
    if resultat == "eau":
        ...
        messagebox.showinfo("Touché", f"{joueur_actif.nom} a touché un bateau !")

    elif resultat == "coule":
        for bateau in joueur_cible.bateaux:
            if bateau.est_coule():
                for i, j in bateau.positions:
                    cx2 = MARGE + j * TAILLE_CASE + 20
                    cy2 = MARGE + i * TAILLE_CASE + 20
                    canvas.create_text(cx2, cy2, text="X", fill="black")
        messagebox.showinfo("Coulé", f"{joueur_actif.nom} a coulé un bateau !")
    ...
    canvas_attaque_1.itemconfigure("all", state="normal")
    canvas_perso_1.itemconfigure("all", state="normal")

    afficher_frame(frame_j1)

    messagebox.showinfo("Tour", f"Au tour de {joueur_actif.nom}")
...
def clic_attaque(event):
    ...
    if 0 <= x < NB_CASES and 0 <= y < NB_CASES:
        ...
        if joueur_actif.simulation:
            window.after(1000, attaque_ordinateur)
